...
def get_order_book_data(symbol="BTCUSDT", limit=20):
    ...
    # 提取买卖盘数据
    # KuCoin 格式: "bids": [["price", "size"], ...], "asks": [["price", "size"], ...]
    # 期望格式: [ {"price": ..., "quantity": ...}, ... ]
    try:
        ...
        if not bids_raw or not asks_raw:
            logger.warning(f"KuCoin订单薄数据中没有有效的买单或卖单")
            return {
                "bids": [],
                "asks": [],
                "bid_ask_ratio": 1.0,
        ...
        bids = [{"price": float(p), "quantity": float(q)} for p, q in bids_raw]
        asks = [{"price": float(p), "quantity": float(q)} for p, q in asks_raw]
    except (ValueError, TypeError) as e:
        logger.error(f"解析 KuCoin 订单薄数据时出错: {e}")
        return {
            "bids": [],
            "asks": [],
            "bid_ask_ratio": 1.0,
    ...
    # 计算买卖比例
    total_bid_volume = sum(item["quantity"] for item in bids)
    total_ask_volume = sum(item["quantity"] for item in asks)
